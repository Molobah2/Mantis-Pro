...
async def run_one_battle(page):
    ...
    # Select Monolith
    try:
        await page.get_by_text("Monolith", exact=False).first.click()
        print("Monolith selected!")
        await page.wait_for_timeout(2000)
    except:
        print("Could not select Monolith")
    ...
    for stage in range(1, 4):
        ...
        if "CRAWL COMPLETE" in content.upper():
            ...
            # Check if crawl complete after continue
            content = await page.inner_text("body")
            if "CRAWL COMPLETE" in content.upper():
                print("  CRAWL COMPLETE!")
                await page.wait_for_timeout(2000)
            ...
            if stage < 3:
                # Scroll to top then bottom to find Next Stage
                await page.wait_for_timeout(3000)
...
async def main():
    print("=" * 40)
    print("LITANY AUTO BATTLE AGENT")
