...
def validate_transfer_nft_input(body: dict) -> Optional[str]:
    """Validate a transfer-nft POST body ({ownerAddress, signature,
    timestamp}) BEFORE any DB touch. Identical shape to
    validate_revoke_grant_input/validate_sweep_grant_input — the mint
    attempt id comes from the URL path, not the body — see
    messages.build_transfer_nft_message."""
    owner_address = body.get("ownerAddress", "")
    if not isinstance(owner_address, str) or not ETH_ADDR_RE.match(owner_address):
        return "Invalid ownerAddress format"
